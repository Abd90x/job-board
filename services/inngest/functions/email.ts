...
import DailyJobListingsEmail from "@/services/resend/components/DailyJobListings";
  ...
  async ({ step, event }) => {
    const getUsers = step.run("get-users", async () => {
      return await db.query.UserNotificationSettingsTable.findMany({
        where: eq(UserNotificationSettingsTable.newJobEmailNotification, true),
        columns: {
          userId: true,
  ...
  async ({ event, step }) => {
    ...
    await step.run("send-email", async () => {
      await resend.emails.send({
        from: "Job Board <onboarding@abdellatif.site>",
        to: user.email,
        subject: "Daily Job Listings",
        react: DailyJobListingsEmail({
          jobListings,
          userName: user.name,
    ...
    async ({ step, event }) => {
      const getUsers = step.run("get-user-settings", async () => {
        return await db.query.OrganizationUserSettingsTable.findMany({
          where: eq(
            OrganizationUserSettingsTable.newApplicationEmailNotification,
            true
          ),
          columns: {
            userId: true,
            ...
            .filter((a) => {
              return settings.find(
                (s) =>
                  ...
                  (s.minRating == null || (a.rating ?? 0) >= s.minRating)
              );
            })
            .map((a) => ({
