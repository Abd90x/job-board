...
import { insertUserNotificationSettings } from "@/features/users/db/userNotificationsSettings";
import {
  deleteOrganization,
  insertOrganization,
  updateOrganization,
...
export const clerkCreateUser = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const userId = await step.run("create-user", async () => {
      ...
      if (!email) throw new NonRetriableError("No email found");
...
export const clerkDeleteUser = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await step.run("delete-user", async () => {
      ...
      if (!id) throw new NonRetriableError("No user id found");
...
export const clerkDeleteOrganization = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await step.run("delete-organization", async () => {
      ...
      if (!id) throw new NonRetriableError("No organization id found");
...
export const clerkCreateOrganizationMember = inngest.createFunction(
  {
...
export const clerkDeleteOrganizationMember = inngest.createFunction(
  {
